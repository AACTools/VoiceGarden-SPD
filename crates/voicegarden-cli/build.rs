fn main() {
    ...
    // has the same block).
    // Passed as a linker argument (not cargo:rustc-link-lib): it must
    // land AFTER ort-sys's rlib on the link line, and build-script
    // link-lib directives are emitted before dependency rlibs.
