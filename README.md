...
- **floravox drives every installed model** (piper/MMS VITS, Matcha, Kokoro): native SSML (`<prosody>`, `<break>`, `<say-as>`, `<sub>`), SpeechMarkdown, and **measured** word timings (from the model's duration tensor) instead of estimates — so index marks and word highlighting track the actual audio. Language routing pulls the published per-language lexicon bundle automatically (the `engines` field in the model registry decides which families are drivable). v0.4.0 removes the sherpa-onnx engine — audio-LM families (kitten, pocket, supertonic, zipvoice) are no longer spoken; the 1,746 drivable families cover the same languages with one onnxruntime in the binary
